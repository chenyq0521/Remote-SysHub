...
void ProcessDialog::on_ProcessTableView_customContextMenuRequested(const QPoint &pos)
{
    ...
    QAction *ProcessHandle = new QAction("进程句柄");
    QAction *ProcessThreads = new QAction("进程线程");
    QAction *ProcessModule = new QAction("进程模块");
    ...
    if(index.isValid())
    {
        ...
        ProcessMenu.addAction(ProcessHandle);
        ProcessMenu.addAction(ProcessThreads);
        ProcessMenu.addAction(ProcessModule);
